...
function Metadata(props, prop_map) {
  ...
  instance.prototype = Metadata.prototype;
...
/**
 * @todo fix prototype inheritance
 * @returns {Array} names all map names
 */
Metadata.prototype.getAllMapNames = function() {
  ...
  if (this.type !== 'Metadata') {
    console.log('Calling method in wrong context');
    return false;
  }
  
  maps = this.get_AllMapsById();
 ...
 * @returns {Array} names all map names
 */
Metadata.prototype.getAllPlaylistNames = function() {
  ...
  playlists = this.get_AllReachPlaylists();
